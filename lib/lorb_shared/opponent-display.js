(function (global) {
    ...
    function createColumnSections(containerFrame, options) {
        ...
        var messageFrame = new Frame(1, 1, containerFrame.width, messageHeight, messageAttr, containerFrame);
        messageFrame.open();
        var loaderFrame = new Frame(1, messageHeight + 1, containerFrame.width, loaderHeight, loaderAttr, containerFrame);
        loaderFrame.open();
        var statusFrame = new Frame(1, messageHeight + loaderHeight + 1, containerFrame.width, statusHeight, statusAttr, containerFrame);
        statusFrame.open();
    ...
    function ensureLayout(options) {
        ...
        var rootFrame = new Frame(spec.x, spec.y, spec.width, spec.height, frameAttr, spec.parent);
        rootFrame.open();
        ...
        var rightWidth = columnWidth;
        var remaining = baseWidth - (leftWidth + rightWidth);
        if (remaining > 0) rightWidth += remaining;
        if (rightWidth <= 0) rightWidth = Math.max(1, baseWidth - leftWidth);

        var leftFrame = new Frame(1, 1, leftWidth, rootFrame.height, columnAttr, rootFrame);
        leftFrame.open();
        var rightFrame = new Frame(leftWidth + 1, 1, rightWidth, rootFrame.height, columnAttr, rootFrame);
        rightFrame.open();

        var leftSections = createColumnSections(leftFrame, options);
        var rightSections = createColumnSections(rightFrame, options);
    ...
    function fallbackConsoleRender(entries) {
        ...
        console.print("\r\nOpponent display unavailable (frame.js missing).\r\n");
        if (!entries.length) {
