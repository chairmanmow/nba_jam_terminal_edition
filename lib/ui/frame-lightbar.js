...
FrameLightbar.prototype._drawItemAt = function(index, viewY) {
    ...
    if (displayText.length > contentWidth) {
        displayText = displayText.substring(0, contentWidth);
        if (hotkeyPos >= contentWidth) hotkeyPos = -1;
    } else if (displayText.length < contentWidth) {
        var padding = contentWidth - displayText.length;
        if (this.align === 1) {
    ...
    if (hotkeyPos >= 0 && hotkeyPos < displayText.length) {
        if (hotkeyPos > 0) {
...
FrameLightbar.prototype._drawDetailLinesAt = function(item, startViewY, isCurrent, isDisabled) {
    ...
    for (var i = 0; i < details.length; i++) {
        ...
        if (!hasCtrlCodes && detailText.length > availWidth) {
            detailText = detailText.substring(0, availWidth);
        }
        
        if (!hasCtrlCodes) {
            var padNeeded = availWidth - detailText.length;
            if (padNeeded > 0) {
