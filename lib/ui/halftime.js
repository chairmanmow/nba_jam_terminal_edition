...
function showHalftimeScreen(systems, mpScreenCoordinator, myPlayerId, coordinator, playerClient) {
    ...
    // NON-COORDINATOR: Apply cached remote substitutions
    // The coordinator broadcasts subs during halftime, and client caches them.
    // Apply the cached subs now before resuming gameplay.
    if (!isCoordinator && playerClient) {
        debugLog("[HALFTIME] Non-coordinator: checking for cached remote substitutions");
        var cachedSubs = playerClient.cachedRemoteSubstitutions;
        if (cachedSubs && cachedSubs.length > 0) {
            debugLog("[HALFTIME] Applying " + cachedSubs.length + " cached remote substitution(s)");
            if (typeof playerClient.applyRemoteSubstitutions === 'function') {
                playerClient.applyRemoteSubstitutions(cachedSubs);
            }
            playerClient.cachedRemoteSubstitutions = null;
        } else {
            debugLog("[HALFTIME] No cached remote substitutions to apply");
        }
    }
