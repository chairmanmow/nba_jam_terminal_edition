    ...
    this.updateShotAnimation = function (anim) {
                ...
                if (blockDist < 4) { // Blocker must be very close
                    ...
                    if (typeof getEffectiveAttribute === "function" && blocker.playerData) {
                        ...
                        if (roll < blockChance) {
                            ...
                            // Stop shot animation early (like dunks) - add a few frames for deflection visual
                            var deflectionFrames = 6;
                            ...
                            // Queue SWAT! stat trail over blocker's head
        ...
        if (anim.step > 0 && trailFrame) {
            ...
            if (anim.blocked && typeof anim.deflectionStartStep === "number" && anim.step > anim.deflectionStartStep) {
                ...
                prevBallX = clamp(Math.round(anim.blockPointX + prevDeflectDx * prevDeflectProgress), 1, COURT_WIDTH);
                prevBallY = clamp(Math.round(anim.blockPointY + prevDeflectDy * prevDeflectProgress - prevDeflectArc), 1, COURT_HEIGHT);
            } else {
                // Normal shot trail
                var prevT = (anim.step - 1) / anim.maxSteps;
                prevBallX = Math.round(anim.startX + (dx * prevT));
                prevBallY = Math.round(anim.startY + (dy * prevT) - (Math.sin(prevT * Math.PI) * arcHeight));
                prevBallX = clamp(prevBallX, 1, COURT_WIDTH);
                prevBallY = clamp(prevBallY, 1, COURT_HEIGHT);
            }

            // Draw trail - RED for blocked shots, normal for regular shots
            var trailAttr;
            if (anim.blocked) {
                // Blocked shot: bright red trail
                trailAttr = LIGHTRED | BG_BLACK;
            } else {
                trailAttr = getOnFireTrailAttr(anim.shooter, anim.step, LIGHTGRAY | WAS_BROWN);
            }
            trailFrame.setData(prevBallX - 1, prevBallY - 1, ".", trailAttr, false);
