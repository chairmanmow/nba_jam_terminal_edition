...
(function() {
    ...
    // Ensure LORB namespace exists (use this. for global scope in Synchronet)
    if (!this.LORB) {
        this.LORB = {};
    }
    if (!this.LORB.Services) {
        this.LORB.Services = {};
    }
    ...
    function LorbNetworkService(ctx, config) {
        ...
        this._usingSharedClient = false;  // True if reusing JsonClientHelper's client
    ...
    LorbNetworkService.prototype.createChallenge = function(toPlayer) {
        ...
        try {
            ...
            this._trackSubscription(DEFAULT_SCOPE, CHALLENGE_ROOT + "." + toPlayer.globalId);
    ...
    LorbNetworkService.prototype.subscribeToGame = function(sessionId) {
        ...
        try {
            ...
            this._trackSubscription(DEFAULT_SCOPE, "game." + sessionId);
            return true;
        } catch (e) {
    ...
    LorbNetworkService.prototype.unsubscribeFromGame = function(sessionId) {
        ...
        try {
            ...
            this._untrackSubscription(DEFAULT_SCOPE, "game." + sessionId);
            return true;
        } catch (e) {
            return false;
        }
    ...
    LorbNetworkService.prototype._connect = function() {
        ...
        try {
            // OPTIMIZATION: Try to reuse the JsonClientHelper's existing connection
            // This reduces connections per user from 2 to 1, avoiding json-service limits
            // See: JSONDB-ARCHITECTURE-AUDIT-v2.md
            if (typeof LORB !== "undefined" && LORB.JsonClientHelper && LORB.JsonClientHelper.ensureClient) {
                var existingClient = LORB.JsonClientHelper.ensureClient({ skipVersionCheck: true });
                if (existingClient && existingClient.connected) {
                    this._client = existingClient;
                    this._usingSharedClient = true;
                    this._log("reusing shared client from JsonClientHelper");
                }
            }
            
            // If no shared client available, create our own
            if (!this._client) {
                this._client = new JSONClient(addr, port);
                this._usingSharedClient = false;
                this._log("created new client to " + addr + ":" + port);
            }
    ...
    LorbNetworkService.prototype._disconnect = function() {
        if (this._client) {
            try {
                ...
                // Only disconnect if we created the client (not shared)
                if (!this._usingSharedClient) {
    ...
    LorbNetworkService.prototype._subscribe = function() {
        ...
        try {
            ...
            this._trackSubscription(DEFAULT_SCOPE, CHALLENGE_ROOT + "." + this._globalId);
            this._log("subscribed to " + CHALLENGE_ROOT + "." + this._globalId);
            
            // Subscribe to presence
            this._client.subscribe(DEFAULT_SCOPE, PRESENCE_ROOT);
            this._trackSubscription(DEFAULT_SCOPE, PRESENCE_ROOT);
     ...
     * Delegates to JsonClientHelper if using shared client
     */
    LorbNetworkService.prototype._trackSubscription = function(scope, path) {
        if (this._usingSharedClient && LORB.JsonClientHelper && LORB.JsonClientHelper.trackSubscription) {
            LORB.JsonClientHelper.trackSubscription(scope, path);
        }
    };
    
    /**
     * Untrack a subscription after manual unsubscribe
     */
    LorbNetworkService.prototype._untrackSubscription = function(scope, path) {
        if (this._usingSharedClient && LORB.JsonClientHelper && LORB.JsonClientHelper.untrackSubscription) {
            LORB.JsonClientHelper.untrackSubscription(scope, path);
        }
    };
