...
(function () {
     ...
     */
    function extractTeamColors(ctx, fallbackBg) {
        var appearance = ctx && ctx.appearance ? ctx.appearance : {};
        
        // Default colors
        var bg = fallbackBg || 4;  // BG_RED = 4 << 4 = 64, but we use raw value
        var fg = 15;  // WHITE
        
        // Try to get jersey color (background)
        if (appearance.jerseyColor && typeof getColorValue === "function") {
            var jerseyBg = getColorValue("BG_" + appearance.jerseyColor);
            if (typeof jerseyBg === "number" && jerseyBg > 0) {
                bg = jerseyBg >> 4;  // Convert from BG value to raw color value
            }
        }
        
        // Try to get jersey lettering (foreground/accent)
        if (appearance.jerseyLettering && typeof getColorValue === "function") {
            var letteringFg = getColorValue(appearance.jerseyLettering);
            if (typeof letteringFg === "number") {
                fg = letteringFg;
            }
        }
    ...
    function buildLorbGameConfig(challengerCtx, challengeeCtx, challenge) {
        ...
        var challengerColors = extractTeamColors(challengerCtx, 4);  // Red fallback
        var challengeeColors = extractTeamColors(challengeeCtx, 1);  // Blue fallback
