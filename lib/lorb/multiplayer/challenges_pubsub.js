...
(function () {
    ...
    // As of Wave 24, this should ALWAYS be true - polling mode caused input freezes
    // See: current_architecture_docs/JSONDB-ARCHITECTURE-AUDIT-v2.md
     ...
     * 
     * This is the correct JSONClient pattern (from json-chat.js):
     * - Subscribe once on connect
     * - Data arrives via handleUpdate callback
     * - cycle() just drains queue (non-blocking)
     */
    ...
    // =========================================================
    // POLL-ON-DEMAND - DEPRECATED
    // =========================================================
    // 
    // WARNING: This function does BLOCKING reads which freeze the input loop!
    // It was used when USE_SUBSCRIPTIONS=false but that architecture is wrong.
    // 
    // The correct pattern (from json-chat.js):
    // - Subscribe once on connect
    // - Data arrives via callback  
    // - cycle() just drains queue (non-blocking)
    // - NEVER call client.read() from input loop
    //
    ...
    /**
     * DEPRECATED - DO NOT USE FROM INPUT LOOP!
     * 
     * Poll for challenges and presence data directly.
     * This does BLOCKING reads that freeze input handling.
     * Only use at startup or in explicit "please wait" screens.
     */
     ...
     * Used in waiting loops - caller should call cycle() + sleep in a loop.
     * 
     * Pattern for waiting:
     *   while (!ready && !timeout) {
     *       mswait(200);
     *       Challenges.cycle();
     *       var ch = Challenges.pollChallenge(id, ctx);
     *       if (ch && isReady(ch)) ready = true;
     *   }
     */
    function pollChallenge(challengeId, ctx) {
        // Just call cycle to process pending updates, then return from cache
        cycle();
     ...
     * 
     * CRITICAL: This function MUST be non-blocking because it's called from
     * the input loop's onIdle callback. Any blocking here freezes input.
     * 
     * Pattern follows json-chat.js:
     * - cycle() just drains socket and updates[] queue
     * - Data arrives via subscription callback
     * - No network reads here!
     */
    function cycle() {
        if (!client || !client.connected) return;
        ...
        // Process incoming packets - this is non-blocking
        // client.cycle() just drains the socket buffer
        processPackets();
        
        // Also drain client.updates[] if callback wasn't invoked
        // (defensive - normally handleUpdate is called via callback)
        while (client.updates && client.updates.length) {
            try {
                handleUpdate(client.updates.shift());
            } catch (e) {
                logWarn("cycle handleUpdate error: " + e);
            }
        }
        
        // Heartbeat: re-broadcast presence periodically (fire-and-forget write)
        if (myGlobalId && myPresenceData && (now - lastHeartbeatTime) >= HEARTBEAT_INTERVAL_MS) {
            ...
            // NO pollOnDemand() - data arrives via subscription callback
        }
    }
    ...
    function listIncoming(ctx) {
        ...
        // Data arrives via subscription callback - no polling!
        cycle();
     ...
     * Data arrives via subscription callback - no polling needed.
     */
