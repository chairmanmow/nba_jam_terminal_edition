...
(function () {
    ...
    /**
     * Do initial data fetch for CHALLENGES ONLY.
     * 
     * Presence is NOT fetched here - it's purely subscription-based.
     * When other players broadcast their heartbeat, we receive it via subscription.
     * This avoids any blocking reads for presence data.
     * 
     * Challenges still need initial fetch because they have longer TTLs and
     * we need to see pending challenges immediately on connect.
     */
    ...
    function fetchInitialData(gid) {
        ...
        var start = nowMs();
        logInfo("fetching initial challenge data (presence is subscription-only)...");
        ...
        try {
            // Fetch my challenge bucket only
            // Presence is NOT fetched - it's purely subscription-based (heartbeat model)
            ...
            // NO PRESENCE FETCH - presence is purely subscription-based
            // Other players' heartbeats arrive via subscription callbacks
            
            logOp("fetchInitialData", "ok", start, "challenges=" + Object.keys(challengeCache).length);
        } finally {
     ...
     * 
     * Also handles heartbeat: if HEARTBEAT_INTERVAL_MS has passed since last
     * presence broadcast, re-broadcast so new subscribers see us.
     */
    function cycle() {
        if (!client || !client.connected) return;
        ...
        if (myGlobalId && myPresenceData && (now - lastHeartbeatTime) >= HEARTBEAT_INTERVAL_MS) {
            ...
            // Update timestamp in presence data
    ...
    /**
     * Write presence with VERY short timeout lock to trigger subscription callbacks.
     * 
     * The JSON service requires lock->write->unlock to notify subscribers.
     * Fire-and-forget writes don't trigger notifications.
     * 
     * Strategy:
     * 1. Always update local cache immediately (so we see ourselves as online)
     * 2. Try lock with very short timeout (250ms)
     * 3. If lock succeeds, write and unlock to notify others
     * 4. If lock fails, no big deal - we'll retry on next heartbeat
     * 
     * This is non-blocking in the sense that we NEVER wait more than 250ms.
     */
    var PRESENCE_WRITE_TIMEOUT_MS = 250;  // Very short - fail fast
    ...
    function writePresence(gid, data) {
        ...
        // Always update local cache immediately (we know we're online)
        if (data) {
        ...
        // Try to broadcast with short-timeout lock
        var oldTimeout = client.settings.TIMEOUT;
        var oldSockTimeout = client.settings.SOCK_TIMEOUT;
        var locked = false;
        ...
        try {
            ...
            logInfo("presence broadcast ok: gid=" + gid);
            return true;
        } catch (e) {
            // Lock timeout or write failure - not fatal
            // Local cache is already updated, we'll retry on next heartbeat
            logInfo("presence broadcast skipped (will retry): " + e);
            return false;
        } finally {
            if (locked) {
    ...
    function clearPresence(ctx) {
        var gid = getGlobalIdFromCtx(ctx);
        if (!gid) return false;
        myPresenceData = null;  // Stop heartbeat
