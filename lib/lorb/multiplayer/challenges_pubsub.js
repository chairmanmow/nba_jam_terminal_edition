...
(function () {
     ...
     */
    var INITIAL_FETCH_TIMEOUT_MS = 2000;
     ...
     */
    var CHALLENGE_WRITE_TIMEOUT_MS = 500;
        ...
        try {
            ...
            if (ch.from && ch.from.globalId) {
                ...
                try {
                    ...
                    client.unlock(DB_SCOPE, fromPath);
                } catch (e) {
                    try { client.unlock(DB_SCOPE, fromPath); } catch (e2) {}
            ...
            if (ch.to && ch.to.globalId) {
                ...
                try {
                    ...
                    client.unlock(DB_SCOPE, toPath);
                } catch (e) {
                    try { client.unlock(DB_SCOPE, toPath); } catch (e2) {}
     ...
     * Short timeout (500ms) ensures we don't block for long on contention.
     */
    var PRESENCE_WRITE_TIMEOUT_MS = 500;
    ...
    function writePresence(gid, data) {
        ...
        try {
            ...
            // Best-effort cleanup - unlock if we got the lock
            try { client.unlock(DB_SCOPE, path); } catch (e2) {}
     ...
     */
    var UPDATE_READ_TIMEOUT_MS = 1000;
