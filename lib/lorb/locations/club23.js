...
(function() {
    ...
    /**
     * Show the Hall of Fame - previous season champions and active playoffs
     */
    function showHallOfFame(view, ctx) {
        ...
        try {
            ...
            // Also get season champions as fallback or additional data
            ...
            // Get all playoff brackets to find active ones
            var playoffBrackets = LORB.Persist.readShared("playoffBrackets");
            if (playoffBrackets) {
                for (var seasonNum in playoffBrackets) {
                    if (playoffBrackets.hasOwnProperty(seasonNum)) {
                        var bracket = playoffBrackets[seasonNum];
                        if (bracket && bracket.status === "active") {
                            activeBrackets.push({
                                seasonNumber: parseInt(seasonNum, 10) || bracket.seasonNumber,
                                currentRound: bracket.currentRound || "unknown",
                                bracketSize: bracket.bracketSize || 4
                            });
                        }
                    }
                }
        ...
        // Sort active brackets by season (oldest first - they should finish first)
        activeBrackets.sort(function(a, b) {
            return (a.seasonNumber || 0) - (b.seasonNumber || 0);
        });
        
        // Combine champions from both sources
        var displayEntries = [];
        var seenSeasons = {};
        
        // First add hall of fame entries (actual Jordan defeats)
        for (var i = 0; i < hallOfFame.length; i++) {
            var hof = hallOfFame[i];
            if (hof.championName && hof.championId) {
                displayEntries.push({
        ...
        // Then add season champions not already in hall of fame
        for (var j = 0; j < seasonChampions.length; j++) {
            var sc = seasonChampions[j];
            if (!seenSeasons[sc.seasonNumber] && sc.championId && sc.championName) {
                // Skip placeholder entries
                if (sc.championName.indexOf("Season") === 0 || sc.championName.indexOf("Reset") !== -1) {
                    continue;
                }
                displayEntries.push({
        ...
        // Sort champions by season descending (newest first)
        displayEntries.sort(function(a, b) {
            return (b.seasonNumber || 0) - (a.seasonNumber || 0);
        });
        
        // Show active playoffs first if any
        if (activeBrackets.length > 0) {
            view.line("  \1h\1cPLAYOFFS IN PROGRESS:\1n");
            for (var ab = 0; ab < activeBrackets.length && ab < 3; ab++) {
                var active = activeBrackets[ab];
                var roundName = active.currentRound || "???";
                // Capitalize first letter
                roundName = roundName.charAt(0).toUpperCase() + roundName.slice(1);
                view.line("  \1n\1c  S" + active.seasonNumber + "\1n \1w- " + roundName + "\1n");
            }
            if (activeBrackets.length > 3) {
                view.line("  \1w  ...+" + (activeBrackets.length - 3) + " more\1n");
            }
            view.blank();
        }
        ...
        if (displayEntries.length === 0) {
            ...
            view.line("  \1h\1yLEGENDS:\1n");
            // Calculate max entries based on active brackets shown
            var maxEntries = activeBrackets.length > 0 ? 5 : 8;
            for (var k = 0; k < Math.min(maxEntries, displayEntries.length); k++) {
                var entry = displayEntries[k];
                var seasonStr = "S" + (entry.seasonNumber || "?");
                var nameColor = entry.defeatedJordan ? "\1h\1y" : "\1h\1w";
                var jordanMark = entry.defeatedJordan ? " \1h\1r*\1n" : "";
                
                view.line("  \1n\1w  " + padRight(seasonStr, 4) + nameColor + entry.name + "\1n" + jordanMark);
            }
            
            if (displayEntries.length > maxEntries) {
                view.line("  \1w  ...+" + (displayEntries.length - maxEntries) + " more\1n");
            }
        }
        
        view.blank();
        view.line("  \1h\1r*\1n \1w= Defeated the Red Bull\1n");
        view.blank();
        view.info("Press any key...");
        view.render();
        console.getkey();
