...
(function () {
    ...
    var GRAFFITI_PATH = "lorb.graffiti";
     ...
     * @param {string} path - JSON-DB path (e.g., "lorb.marriages")
    ...
    function readShared(path) {
        ...
        try {
            var data = client.read(DB_SCOPE, path, LOCK_READ);
            logDb("readShared", path, "ok", start, data ? "hasData" : "empty");
            return data;
        } catch (e) {
            logDb("readShared", path, "error", start, e);
            log(LOG_WARNING, "LORB: Error reading shared path " + path + ": " + e);
            return null;
        }
    }
    ...
    function writeShared(path, data) {
        ...
        try {
            ...
            logDb("writeShared", path, "ok", start);
            return true;
        } catch (e) {
            logDb("writeShared", path, "error", start, e);
            log(LOG_ERR, "LORB: Error writing shared path " + path + ": " + e);
            return false;
        }
    }
    ...
    function readGraffiti() {
        ...
        try {
            var data = client.read(DB_SCOPE, GRAFFITI_PATH, LOCK_READ);
            logDb("graffiti.read", GRAFFITI_PATH, "ok", start, data ? "count=" + (data.entries ? data.entries.length : 0) : "empty");
            
            if (!data || !data.entries) return [];
            return data.entries;
        } catch (e) {
            logDb("graffiti.read", GRAFFITI_PATH, "error", start, e);
            log(LOG_WARNING, "LORB: Error reading graffiti: " + e);
            return [];
        }
    }
    ...
    function addGraffiti(ctx, lines) {
        ...
        for (var i = 0; i < lines.length && i < maxLines; i++) {
            ...
            // Strip control codes for safety (except basic color codes)
        ...
        try {
            ...
            // Write back
            client.write(DB_SCOPE, GRAFFITI_PATH, { entries: entries }, LOCK_WRITE);
            logDb("graffiti.add", GRAFFITI_PATH, "ok", start, "total=" + entries.length);
            log(LOG_DEBUG, "LORB: Added graffiti by " + entry.author);
            return true;
        } catch (e) {
            logDb("graffiti.add", GRAFFITI_PATH, "error", start, e);
            log(LOG_ERR, "LORB: Error adding graffiti: " + e);
            return false;
        }
    }
