...
(function() {
    ...
    function renderToFrame(frame, text, fgColor, bgColor, maxWidth, maxHeight) {
        ...
        try {
            if (!teamColor && typeof LORB !== "undefined" && LORB.Cities && LORB.Cities.getToday) {
            ...
            // Ignore color lookup errors, proceed with default
            teamColor = null;
        }
        
        var rendered = null;
        try {
            rendered = renderRandom(text, maxWidth, maxHeight, teamColor);
        } catch (e) {
            // Font rendering failed, fall through to fallback
            rendered = null;
        }
        ...
        try {
            frame.clear();
            frame.gotoxy(1, 1);  // Explicitly position at top-left (1-indexed)
            
            // Strip any leading newlines from TDF output
            while (rendered.charAt(0) === '\n' || rendered.charAt(0) === '\r') {
                rendered = rendered.substring(1);
            }
            
            frame.putmsg(rendered);
        } catch (e) {
            // If frame operations fail, return false
            return false;
        }
