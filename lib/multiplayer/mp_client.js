...
function PlayerClient(sessionId, client, myPlayerId, serverConfig, systems) {
    ...
    this.cachedRemoteSubstitutions = null; // Cache subs received during halftime
    ...
    this.updateGameState = function (state) {
        ...
        if (typeof state.ht === "boolean") {
            ...
            if (!wasHalftime && isHalftime) {
                ...
                this.cachedRemoteSubstitutions = null; // Clear cache on halftime start
                ...
                // Apply cached substitutions if we have them
                if (!this.isCoordinator && this.cachedRemoteSubstitutions && this.cachedRemoteSubstitutions.length > 0) {
                    debugLog("[MP CLIENT] Applying " + this.cachedRemoteSubstitutions.length + " cached substitution(s)");
                    this.applyRemoteSubstitutions(this.cachedRemoteSubstitutions);
                    this.cachedRemoteSubstitutions = null;
                }
            }
            
            // Cache substitutions while in halftime (coordinator may clear before we exit)
            if (isHalftime && !this.isCoordinator && state.subs && state.subs.length > 0) {
                debugLog("[MP CLIENT] Caching " + state.subs.length + " substitution(s) during halftime");
                this.cachedRemoteSubstitutions = state.subs;
            }
        } else {
    ...
    this.applyRemoteSubstitutions = function (subs) {
        ...
        for (var i = 0; i < subs.length; i++) {
            ...
            try {
                ...
                debugLog("[MP CLIENT] Applied remote substitution: " + teamKey + " slot " + slot + " -> " + playerInfo.name);
