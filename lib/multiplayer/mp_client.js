    ...
    this.updateGameState = function (state) {
        ...
        if (typeof state.ht === "boolean") {
            ...
            if (!wasHalftime && isHalftime) {
                ...
                // Apply any pending substitutions from coordinator
                // Only clients need to apply remote subs - coordinator already has the correct sprites
                if (!this.isCoordinator && state.subs && state.subs.length > 0) {
                    debugLog("[MP CLIENT] Processing " + state.subs.length + " substitution(s) from coordinator");
                    this.applyRemoteSubstitutions(state.subs);
                }
            }
        } else {
