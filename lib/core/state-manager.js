...
function createStateManager(initialState) {
    ...
    function set(path, value, reason) {
        ...
        // DEBUG: Log all ballCarrier changes (avoid circular refs)
        if (path === 'ballCarrier' && typeof log === 'function') {
            var oldName = "null", newName = "null";
            try {
                if (oldValue && oldValue.playerData) oldName = oldValue.playerData.name;
                if (value && value.playerData) newName = value.playerData.name;
            } catch (e) { /* ignore */ }
            log(LOG_DEBUG, "[STATE-MANAGER] Setting ballCarrier: " + oldName + " -> " + newName + " (reason: " + (reason || "none") + ")");
        }
        ...
        // DEBUG: Verify mutation happened
        if (path === 'ballCarrier' && typeof log === 'function') {
            var verifyName = "null";
            try {
                if (state.ballCarrier && state.ballCarrier.playerData) {
                    verifyName = state.ballCarrier.playerData.name;
                }
            } catch (e) { /* ignore */ }
            log(LOG_DEBUG, "[STATE-MANAGER] After mutation, state.ballCarrier is: " + verifyName);
        }
