...
function createStateManager(initialState) {
    ...
    function set(path, value, reason) {
        ...
        // DEBUG: Log all ballCarrier changes
        if (path === 'ballCarrier' && typeof log === 'function') {
            log(LOG_DEBUG, "[STATE-MANAGER] Setting ballCarrier: " + 
                (oldValue ? oldValue.playerData.name : "null") + " -> " + 
                (value ? value.playerData.name : "null") + 
                " (reason: " + (reason || "none") + ")");
        }
        ...
        if (path === 'ballCarrier' && typeof log === 'function') {
            log(LOG_DEBUG, "[STATE-MANAGER] After mutation, state.ballCarrier is: " + 
                (state.ballCarrier ? state.ballCarrier.playerData.name : "null"));
        }
