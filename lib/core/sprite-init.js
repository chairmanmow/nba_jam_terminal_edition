...
function initSpritesFromDynamic(teamADef, teamBDef, options, systems) {
    ...
    if (lorbContext && lorbContext.crew && Array.isArray(lorbContext.crew)) {
        ...
        for (var k = 0; k < lorbContext.crew.length; k++) {
            var crewMember = lorbContext.crew[k];
            var crewContact = null;
            
            // crew entries can be { contactId: id } or full contact objects
            if (crewMember.contactId && lorbContext.contacts) {
                // Look up contact by ID
                for (var m = 0; m < lorbContext.contacts.length; m++) {
                    if (lorbContext.contacts[m].id === crewMember.contactId) {
                        crewContact = lorbContext.contacts[m];
                        break;
                    }
                }
