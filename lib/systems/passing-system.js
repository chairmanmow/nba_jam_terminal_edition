 ...
 */
    ...
    function _handlePassComplete(stateData) {
        // DEBUG: Log callback execution (avoid circular refs)
        if (typeof log === 'function') {
            var receiverName = "null";
            try {
                if (stateData.receiver && stateData.receiver.playerData) {
                    receiverName = stateData.receiver.playerData.name;
                }
            } catch (e) {
                receiverName = "error";
            }
            log(LOG_DEBUG, "[PASSING-SYSTEM] _handlePassComplete fired! intercepted=" + stateData.intercepted + ", receiver=" + receiverName);
        }
        ...
        deps.state.set('reboundActive', false, 'pass_complete'); if (stateData.intercepted && stateData.receiver) {
            ...
            if (typeof log === 'function') {
                ...
                try {
                    if (stateData.receiver && stateData.receiver.playerData) {
                        intName = stateData.receiver.playerData.name;
                    }
                } catch (e) { intName = "error"; }
                log(LOG_DEBUG, "[PASSING-SYSTEM] Setting ballCarrier to interceptor: " + intName);
            }
            ...
            if (typeof log === 'function') {
                ...
                try {
                    if (stateData.receiver && stateData.receiver.playerData) {
                        recName = stateData.receiver.playerData.name;
                    }
                } catch (e) { recName = "error"; }
                log(LOG_DEBUG, "[PASSING-SYSTEM] Setting ballCarrier to receiver: " + recName);
            }
            ...
            if (typeof log === 'function') {
                var currentCarrier = deps.state.get('ballCarrier');
                var carrierName = "NULL";
                try {
                    if (currentCarrier && currentCarrier.playerData) {
                        carrierName = currentCarrier.playerData.name;
                    }
                } catch (e) { carrierName = "error"; }
                log(LOG_DEBUG, "[PASSING-SYSTEM] After set, ballCarrier is: " + carrierName);
            }
